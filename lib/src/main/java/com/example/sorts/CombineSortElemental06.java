...
/**
 * <p>
 * In-Place, Stable, Parallelizable sorting algorithm
 * </p>
 * 
 * <p>
 * Originally intended to be a  <i> classic </i> implementation. 
 * However, requires insertion-sort to handle edge-cases. 
 * Also incorporated wedge-sort (an improvement over insertion-sort) to avoid overflow due to recursion while merging small subsets into much larger ones. 
 * Overflow is typically not seen when insertion sort is used (for random / reverse elements upto 100M) but there is still a possibility and as such is retained.
 * </p>
 * 
 * @author ravindra-hasyagar
 * @since 09,10-Jan-2026
 */
public class CombineSortElemental06 {
	...
	/*
	 * --//--use-wedge-sort-if-recursion-may-go-over-this-threshold-as-determined-by-the-split-ratio
	 */
	private static final int WEDGE_SORT_RECURSION_CRITERIA =  128;
...
//		handleTest(input14);
	...
	private static void testOne() {
		...
		int[] inputSizes = { 1, 2, 3, 4, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 100, 1000, 2000, 3000, 4000 };
		int iterations = 100000;
		SecureRandom random = new SecureRandom();
		
		for(int i=0;i<iterations;i++) {
			
			int[] arrTemp = new int[inputSizes[random.nextInt(inputSizes.length)]];
			
			for (int j = 0; j < arrTemp.length; j++) {
				arrTemp[j] = random.nextInt(arrTemp.length);
//				arrTemp[j] = (arrTemp.length - i) * ((i % 2 == 0) ? (-1) : (1));
//				arrTemp[j] = 101;
//				arrTemp[j] = Integer.MAX_VALUE - i;
			}
	...
	private static void handleTest(int[] arr) {
		...
		if (!success) {
			...
			System.out.println("Sort-Success !! Len - " + arr.length + ", Iteration : " + ITERATIONS.incrementAndGet());
		}
	...
	private static void merge(int[] arr, int low, int mid, int high) {
		...
		*/
		...
		double ratioLHS = (diffAbs) / diffLHS * (1.0);
		double ratioRHS = (diffAbs) / diffRHS * (1.0);
		
		if((ratioLHS > WEDGE_SORT_RECURSION_CRITERIA))  {
			WedgeSort.handleRightWedge(arr, low, mid - 1, high - 1);
			return;
		}

		if ((ratioRHS > WEDGE_SORT_RECURSION_CRITERIA)) {
			WedgeSort.handleLeftWedge(arr, low, mid, high - 1);
			return;
		}
		int offset = 0;
